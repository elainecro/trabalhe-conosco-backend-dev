(function () {
    angular.module('picpayApp').factory('auth', [
        '$http',
        'consts',
        AuthFactory
    ])
    ...
    function AuthFactory($http, consts) {
        ...
        function signin (login, callback) {
            submit('signin', login, callback)
        }
                ...
                $http.post(`${consts.oapiUrl}/validateToken`, { token })
                    .then(resp => {
                        if (!resp.data.valid) {
                            ...
                            $http.defaults.headers.common.Authorization = getLogin().token
                        }
        ...
        return { signup, signin, logout, getLogin, validateToken }
    }
})()